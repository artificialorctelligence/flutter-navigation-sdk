...
class AutoMapViewAPIImpl {
  ...
  Future<List<TileOverlay?>> getTileOverlays() async {
  ...
  Future<void> removeTileOverlays({
    ...
    final List<TileOverlayDto> dtos = tileOverlays
        .map((TileOverlay tileOverlay) => tileOverlay.toDto())
        .toList();
    return _viewApi.removeTileOverlays(dtos).wrapPlatformException();
  }

  /// Remove all tile overlays from map view.
  Future<void> clearTileOverlays() =>
      _viewApi.clearTileOverlays().wrapPlatformException();

  /// Drop cached tiles for one overlay, so a layer whose tiles change over time can refresh.
  Future<void> clearTileCache({required String tileOverlayId}) =>
      _viewApi.clearTileCache(tileOverlayId).wrapPlatformException();
